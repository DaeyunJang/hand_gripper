    ...
    try:
        ...
        while True:
            ...
            if cmd == "reboot":
                ...
                # 사용할 ID 필터 (20~52)
                target_ids = [mid for mid in dxl.motor_ids if 0 <= mid <= 52]

                # center / amplitude 계산
                center = {}
                amp = {}
                for mid in target_ids:
                    c = (open_dict[mid] + grip_dict[mid]) / 2
                    a = (grip_dict[mid] - open_dict[mid]) / 2
                    center[mid] = c
                    amp[mid] = a
                    ...
                    while True:
                        ...
                        for mid in dxl.motor_ids:
                            if mid in target_ids:
                                pos = int(center[mid] + amp[mid] * math.sin(omega * t))
                            else:
                                # ID 10,11,12 유지
                ...
                # 20~52만 사용
                target_ids = [mid for mid in dxl.motor_ids if 10 <= mid <= 52]
                ...
                amp[12] = amp[12] *0.5
                # 손가락별 위상차 [deg]
                # finger 2,3,4,5만 실제 반영됨. 10대는 제외했으니까.
                finger_phase_deg = {
                    1: 0.0,
                    2: 0.0,
                    3: 30.0,
                ...
                try:
                    while True:
                        ...
                        for mid in dxl.motor_ids:
                            if mid in target_ids:
                                finger_idx = mid // 10
                                phase = math.radians(finger_phase_deg.get(finger_idx, 0.0))
                                pos = int(center[mid] + amp[mid] * math.sin(omega * t + phase))
                            else:
                                # 10,11,12는 반영 안 함 → 현재 위치 유지 대신 hand_open 유지
                                pos = open_dict.get(mid, 2048)
