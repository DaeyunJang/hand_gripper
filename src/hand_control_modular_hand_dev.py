...
def extract_finger_configs(hand_cfg: dict) -> list[tuple[str, dict]]:
    if not isinstance(hand_cfg, dict):
        raise ValueError("'hand' config must be a dictionary.")

    if "fingers" in hand_cfg:
        fingers_cfg = hand_cfg["fingers"]

        if isinstance(fingers_cfg, list):
            result = []
            for idx, finger_data in enumerate(fingers_cfg):
                if not isinstance(finger_data, dict):
                    raise ValueError(f"Finger entry at index {idx} must be a dictionary.")
                finger_name = finger_data.get("name", f"finger_{idx}")
                result.append((finger_name, finger_data))
            return result

        if isinstance(fingers_cfg, dict):
            result = []
            for finger_name, finger_data in fingers_cfg.items():
                if not isinstance(finger_data, dict):
                    raise ValueError(f"Finger config for '{finger_name}' must be a dictionary.")
                result.append((finger_name, finger_data))
            return result

        raise ValueError("'hand.fingers' must be either a list or a dictionary.")

    result = []
    for i in range(1, hand_cfg.get("num_of_fingers", 0) + 1):
        finger_key = f"finger_{i}"
        if finger_key in hand_cfg:
            result.append((finger_key, hand_cfg[finger_key]))
    return result
...
class DynamixelController:
    def __init__(self, verbose: bool = True):
        ...
        self.motor_ids = []
        self.finger_map = {}
        self.motor_to_finger_name = {}
        self.finger_order = []

        finger_entries = extract_finger_configs(config["hand"])
        for idx, (finger_name, finger_data) in enumerate(finger_entries):
            finger_ids = finger_data.get("ids", [])
            self.finger_order.append(finger_name)
            self.finger_map[finger_name] = {
                "index": idx,
                "ids": finger_ids,
            }
            self.motor_ids.extend(finger_ids)
            for motor_id in finger_ids:
                self.motor_to_finger_name[motor_id] = finger_name
    ...
    def apply_motion(self, same_eta=True):
        with self.lock:
            ...
            self.groupSyncWriteVelocity.txPacket()
    ...
    def get_states(self, verbose=True):
        with self.lock:
            ...
            for motor_id in self.motor_ids:
                if not self.groupSyncReadPosition.isAvailable(motor_id, control_table["ADDR_PRESENT_POSITION"], control_table["LEN_PRESENT_POSITION"]):
                    print(f"[ID:{motor_id}] groupSyncRead getdata failed")
                    continue
                if not self.groupSyncReadVelocity.isAvailable(motor_id, control_table["ADDR_PRESENT_VELOCITY"], control_table["LEN_PRESENT_VELOCITY"]):
                    print(f"[ID:{motor_id}] groupSyncReadVelocity getdata failed")
                    continue
                if not self.groupSyncReadCurrent.isAvailable(motor_id, control_table["ADDR_PRESENT_CURRENT"], control_table["LEN_PRESENT_CURRENT"]):
...
def main_control_loop():
    dxl = None
    try:
        dxl = DynamixelController(verbose=True)
        while True:
            ...
            if cmd == "reboot":
                ...
                reload_motion_table()
            elif cmd == "sine_palm_v2":
                dxl.execute_motion("hand_open")
                time.sleep(1)

                import math
                print("Start sine motion... (Ctrl+C to stop)")

                open_data = motion_table["motion"]["hand_open"]
                grip_data = motion_table["motion"]["palm_grip"]

                open_dict = {m["ID"]: m["Position"] for m in open_data}
                grip_dict = {m["ID"]: m["Position"] for m in grip_data}

                target_ids = [mid for mid in dxl.motor_ids if dxl.motor_to_finger_name.get(mid) != "thumb"]

                center = {}
                amp = {}
                for mid in target_ids:
                    center[mid] = (open_dict[mid] + grip_dict[mid]) / 2
                    amp[mid] = (grip_dict[mid] - open_dict[mid]) / 2
                ...
                try:
                    while True:
                        ...
                        for mid in dxl.motor_ids:
                            finger_name = dxl.motor_to_finger_name.get(mid, "")

                            if mid in target_ids:
                                phase = math.radians(finger_phase_deg.get(finger_name, 0.0))
                                pos = int(center[mid] + amp[mid] * math.sin(omega * t + phase))
                            else:
                                pos = open_dict.get(mid, 2048)

                            positions.append(pos)
                            currents.append(200)

                        dxl.set_goal_positions(positions)
                        dxl.set_goal_current(currents)
                        dxl.apply_motion()

                        time.sleep(0.02)

                except KeyboardInterrupt:
                    print("Sine motion stopped.")
            else:
                try:
